...
class Registration extends MY_Controller
{
	...
	public function index()
	{
		$head = array();
		$navbar = array();
		$head['title'] = 'Ecommerce-registration';
		...
		$this->load->view('ecommerce/login/registration');
	...
	public function process_registration()
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$error_message = 'Invalid, Registration Fail.';
			$this->session->set_flashdata('message', $error_message);
			redirect('ecommerce/registration');
			...
			if ($this->Registration_model->is_email_unique($email)) {
				...
				// $this->load->view('ecommerce/login');
			} else {
				// Email already exists, display an error message
				$error_message = 'Email address already in use.';
				$this->session->set_flashdata('message', $error_message);
				redirect(base_url('ecommerce/registration'));
